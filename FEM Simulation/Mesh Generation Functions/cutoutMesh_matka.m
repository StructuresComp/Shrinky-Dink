function [FEMesh, cutOutElements_kir, cutOutElements_sub_bottom, cutOutElements_sub_top]...
    = cutoutMesh_matka(innerRadius, midRadius,outerRadius, ...
    Nstrips, dTheta, ...
...
% plot(ptQuads_x,ptQuads_y);

ptQuads_x2 = zeros(Nstrips, (2*Ndiscrete));
ptQuads_y2 = zeros(Nstrips, (2*Ndiscrete));

for c=1:Nstrips
    th_1 = theta(c) + dTheta/2;
    th_2 = theta(c) + 3*dTheta/2;
    ...
    % Store
    ptQuads_x2(c, 1:Ndiscrete) = innerEdge_x;
    ptQuads_x2(c, Ndiscrete+1:2*Ndiscrete) = outerEdge_x;
    ptQuads_y2(c, 1:Ndiscrete) = innerEdge_y;
    ptQuads_y2(c, Ndiscrete+1:2*Ndiscrete) = outerEdge_y;
end
plot(ptQuads_x2,ptQuads_y2);
...
gd = zeros(gMatLength, 2*Nstrips+2);
...
for c=Nstrips+1:2*Nstrips
    gd(1, c) = 2;
    gd(2, c) = numel(ptQuads_x2(c-Nstrips,:));
    
    counter_i = 3;
    counter_f = 2 + numel(ptQuads_x2(c-Nstrips,:));
    gd(counter_i:counter_f, c) = ptQuads_x2(c-Nstrips,:);

    counter_i = counter_f + 1;
    counter_f = (counter_i-1) + numel(ptQuads_y2(c-Nstrips,:));
    gd(counter_i:counter_f, c) = ptQuads_y2(c-Nstrips, :);
...
circlePartNo = 1;
minDist = midRadius*10;
add_to_kir_array = [];
add_to_sub_array_top = [];
add_to_sub_array_bottom = [];
for c=[Nstrips+1:Nstrips+10 Nstrips+15:Nstrips+18 Nstrips+21:Nstrips+22 Nstrips+25]
    % Check if there is an element that is very close to zero
    add_to_sub_array_bottom = [add_to_sub_array_bottom;c];
end

for c=[1:Nstrips Nstrips+9 Nstrips+15:Nstrips+18 Nstrips+21:Nstrips+22 Nstrips+25] 
    % Check if there is an element that is very close to zero
    add_to_sub_array_top = [add_to_sub_array_top;c];
end

for c=[1:Nstrips Nstrips+9:Nstrips+10 Nstrips+15:Nstrips+18 Nstrips+21:Nstrips+22 Nstrips+25]
    % Check if there is an element that is very close to zero

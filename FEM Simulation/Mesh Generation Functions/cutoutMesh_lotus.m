function [FEMesh, cutOutElements_kir] = cutoutMesh_lotus(innerRadius, outerRadius, ...
    Nstrips, dTheta, ...
...
%% Mesh it
FEMesh = generateMesh(model,'GeometricOrder','quadratic','Hmin',minMeshSize,'Hmax',maxMeshSize);

pdemesh(FEMesh.Nodes, FEMesh.Elements,'EdgeColor','green');
%% Identify which region is the main circular part
cutOutElements_kir = findElements(FEMesh,'region','Face',Nstrips + 1);

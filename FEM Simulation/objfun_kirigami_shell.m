function objfun_kirigami_shell(shape, outerRadius, stress, template_file,...
...
% Name of the job
jobName = sprintf( [inputNameString, '%s'], shape);
...
inputName = sprintf( [inputNameString, '%s.inp'], shape);
...
nodesFile = sprintf( [inputNameString,'%s-nodes.inp'], shape);

% Boundary condition file
bcFile = sprintf( [inputNameString,'%s-boundaryCondition.inp'],  shape);
%% Create an input (.inp) file
write_input_file(inputName, nodesFile, bcFile,template_file, mat_param);

%% Populate the files required
meshGen_shell(shape, outerRadius, nodesFile, bcFile, stress,...
    thickness_sub, thickness_kir,...
    FEMesh, cutOutElements_kir);
%%
Cmd1 = [abaqus_addr,' j=', jobName, ' input=', inputName, ' double &'];

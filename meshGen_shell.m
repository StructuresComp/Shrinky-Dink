function meshGen_shell(nodesFile, bcFile,...
     temp_in, thickness_1, thickness_2,...
     FEMesh, cutOutElements,mat_param, trilayer)
...
% nLayers = 3; % Number of sub-layers per layer
...
for i=1:nLayers
    fprintf(fid, '*Elset, elset = elBot%d, generate\n',i); % all elements in the substrate layer
    fprintf(fid,'%d, %d, 1\n',num_el_kir*(i-1)+1, num_el_kir*i);
end
...
xDesired = FEMesh.MinElementSize*1.001;
yDesired = 0.0;
dist = sqrt( (x2D - xDesired).^2 + (y2D - yDesired).^2 );
[~, minInd2] = min(dist);
fprintf(fid, '*Nset, nset=orientationNode\n%d\n',minInd2);

yDesired = FEMesh.MinElementSize*1.001;
xDesired = 0.0;
dist = sqrt( (x2D - xDesired).^2 + (y2D - yDesired).^2 );
[~, minInd2] = min(dist);
fprintf(fid, '*Nset, nset=besideNode\n%d\n',minInd2);

 fprintf(fid, '*Solid Section, elset=elSubstrate, material=Material-Substrate, orientation=OR1\n');
for i=1:nLayers
    fprintf(fid, '*Solid Section, elset=elBot%d, material=Material-Kirigami%d, orientation=OR1\n',i,i);
end

if trilayer==1
    fprintf(fid, '*Solid Section, elset=elTop, material=Material-Kirigami, orientation=OR1\n');   
end
